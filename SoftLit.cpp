...
int main(int argc, char* argv[])
{
    ...
    mat4 proj = perspective(glm::radians(fov), (float)width / (float)height, 0.125f, 1024.f);
    ...
    while (running)
    {
        ...
        //printf("Display time: %lld ms\n", std::chrono::duration_cast<std::chrono::milliseconds> (presentEnd - presentBegin).count());

        printf("Frame time: %lld ms\n", std::chrono::duration_cast<std::chrono::milliseconds> (presentEnd - drawBegin).count());
    }
#endif
...
void ImportOBJ(vector<Primitive*>& objs, const string& filename)
{
    ...
    if (ret)
    {
        ...
        for (size_t i = 0; i < shapes.size(); i++)
        {
            ...
            VertexBuffer& vbo = obj->getVertexBuffer();
            IndexBuffer& ibo = obj->getIndexBuffer();
